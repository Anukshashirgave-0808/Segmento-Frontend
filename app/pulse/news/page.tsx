...
const categoryRelationships: Record<string, Array<{ id: string; name: string }>> = {
  ...
  // repeat all other cloud-* categories with same structure as above...
...
function NewsContent() {
  ...
  useEffect(() => {
    const getNews = async () => {
      ...
      if (cached && (cached as any)._timestamp && Date.now() - (cached as any)._timestamp < 120000) {
        return; // Use cached data if less than 2 mins old
      }
